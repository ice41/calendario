...
import { Users, Calendar, Clock, CheckCircle, Pencil, RotateCcw, ArrowUpRight, AlertCircle, ChevronRight } from 'lucide-react';
...
export function Dashboard({ onNavigate }: { onNavigate?: (tab: 'dashboard' | 'employees' | 'calendar') => void }) {
    ...
    const handleEditSubmit = (data: Omit<VacationRequest, 'id' | 'companyId'>) => {
        if (editingVacation) {
            updateVacation({
                ...data,
                id: editingVacation.id,
                companyId: editingVacation.companyId,
            } as VacationRequest);
        }
