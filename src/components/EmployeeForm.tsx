...
interface EmployeeFormProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (employee: Omit<Employee, 'id' | 'companyId'>) => Promise<void>;
...
export function EmployeeForm({ isOpen, onClose, onSubmit, initialData }: EmployeeFormProps) {
    const { employees, roles, departments } = useApp();
    const [isSubmitting, setIsSubmitting] = useState(false);

    // Default to the first role available, or empty string if none exists.
    const defaultRole = roles.length > 0 ? roles[0].name : '';
    // Default to the first department available
    ...
    }, [initialData, isOpen, defaultRole, defaultDept, departments]);
